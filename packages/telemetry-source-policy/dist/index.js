...
function isSelfContext(context, self) {
    if (SELF_CONTEXTS.has(context)) {
        return true;
    }
    return Boolean(context && self && context === self);
}
function isStaticDefaultsPath(path) {
    return (path === 'name' ||
...
function isVictronPath(path) {
    return (path.startsWith('electrical.batteries.tideyeBmv.') ||
...
function isNavigationPrimaryPath(path) {
    return (path === 'navigation.position.latitude' ||
        path === 'navigation.position.longitude' ||
        path === 'navigation.speedOverGround' ||
        path === 'navigation.courseOverGroundTrue');
}
function isWindPath(path) {
    return path.startsWith('environment.wind.');
}
function isCurrentPath(path) {
    return path.startsWith('environment.current.');
}
function isDepthTankBatteryPath(path) {
    return (path.startsWith('environment.depth.') ||
        path.startsWith('tanks.') ||
        /^electrical\.batteries\.[^.]+\./.test(path));
}
function getPathFreshnessWindowMs(path) {
    if (isFastPath(path)) {
        return 15_000;
    }
...
function getNormalizedReceivedAtMs(receivedAt) {
...
function getSourceRank(input) {
    ...
    if (/^propulsion\.[^.]+\.(runTime|runTimeTrip)/.test(canonicalPath) &&
        normalizedSourceId.startsWith('signalk-engine-hours.')) {
        return primaryPenalty;
    }
    if (AVERAGE_PLUGIN_PATHS.has(canonicalPath) &&
...
function compareRankedCandidates(left, right, stickyWinner) {
    if (left.rank !== right.rank) {
        return left.rank - right.rank;
    }
    if (stickyWinner) {
        const leftSticky = left.sourceId === stickyWinner.sourceId && left.publisherRole === stickyWinner.publisherRole
            ? 0
            : 1;
        const rightSticky = right.sourceId === stickyWinner.sourceId && right.publisherRole === stickyWinner.publisherRole
            ? 0
            : 1;
...
function toRankedCandidate(candidate) {
    return {
        ...candidate,
...
function getStickyRank(stickyWinner, groupCandidate) {
    return (stickyWinner.precedenceRank ??
        getSourceRank({
...
function buildGroupKey(context, canonicalPath) {
    return `${context}\u0000${canonicalPath}`;
}
function addExpandedLeaf(leaves, originalPath, canonicalPath, value) {
...
function expandObjectValue(originalPath, currentPath, value, leaves) {
    if (!isRecord(value)) {
...
export function classifySignalKSourceFamily(sourceId) {
...
export function expandTelemetryValueLeaves(input) {
    const leaves = [];
...
export function expandSignalKLeafValues(path, value) {
...
export function selectCanonicalTelemetry(input) {
    ...
    const stickyMap = input.stickyWinners instanceof Map
        ? input.stickyWinners
    ...
    for (const [key, group] of grouped.entries()) {
        ...
        const stickyFresh = stickyWinner && now - getNormalizedReceivedAtMs(stickyWinner.receivedAt) <= freshnessWindowMs
            ? stickyWinner
            : null;
        const ranked = group
        ...
        if (stickyFresh) {
            ...
            const shouldRetainSticky = stickyRank < winner.rank ||
            ...
            if (shouldRetainSticky) {
                const shadowSuppressed = stickyFresh.publisherRole === 'primary' &&
                    ranked.every((candidate) => candidate.publisherRole === 'shadow');
                const reason = shadowSuppressed
        ...
        results.push({
            ...
            dropped: ranked.slice(1).map((candidate) => ({
                candidate,
                reason: winner.publisherRole === 'primary' && candidate.publisherRole === 'shadow'
                    ? 'shadow_source_suppressed'
                    : 'lower_priority_source',
            })),
            fallbackToLowerPriority: Boolean(stickyWinner &&
                stickyWinner.sourceId !== winner.sourceId &&
                getStickyRank(stickyWinner, winner) < winner.rank),
            key,
            shadowSourceSuppressed: winner.publisherRole === 'primary' &&
...
export function buildStickyWinnerMap(results) {
    const stickyWinners = new Map();
    for (const result of results) {
        const winner = 'winner' in result ? result.winner : (result.kept ?? null);
        if (!winner) {
            continue;
        }
        const key = 'key' in result && typeof result.key === 'string'
            ? result.key
...
export function buildDeltaFromCandidates(input) {
    ...
    const updates = Array.from(input.candidates.reduce((groups, candidate) => {
        const groupKey = [
            candidate.sourceId,
...
export function normalizeTelemetryPathFamily(path) {
...
export function getSelectionPolicySummary(results) {
...
export function selectTelemetryCandidates(input) {
    const now = typeof input.now === 'string' ? getNormalizedReceivedAtMs(input.now) : (input.now ?? Date.now());
    return selectCanonicalTelemetry({
        candidates: input.candidates,
        now,
        stickyWinners: input.stickyWinners,
    }).map((result) => {
        const rejected = result.dropped.map(({ candidate, reason }) => ({
            candidate,
            reason: reason === 'sticky_winner_retained' ? 'sticky_winner_fresh' : reason,
            sourceId: candidate.sourceId,
        }));
        return {
            canonicalPath: result.winner?.canonicalPath || result.dropped[0]?.candidate.canonicalPath || '',
