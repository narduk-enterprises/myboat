...
const nuxtPort = Number(process.env.NUXT_PORT || process.env.PLAYWRIGHT_NUXT_PORT || 3400)
...
export default defineConfig({
  ...
  webServer: {
    command: `pnpm --filter web run db:ready && cd apps/web && NUXT_PORT=${nuxtPort} nuxt dev`,
    url: baseURL,
