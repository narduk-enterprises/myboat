  ...
  <div class="space-y-8">
    <template v-if="pending">
      ...
      <div class="grid gap-5 lg:grid-cols-2">
        <USkeleton v-for="item in 4" :key="item" class="h-64 rounded-[1.75rem]" />
      </div>
    </template>
    ...
    <template v-else-if="overview">
          ...
          <div class="space-y-4">
            ...
            <div>
              ...
              <p class="mt-3 max-w-3xl text-base leading-7 text-muted">
                Profile identity, security, local display preferences, and sharing policy stay here
                so onboarding can stay focused on the first production setup.
              </p>
            </div>
          </div>
      ...
      <section class="grid gap-5 lg:grid-cols-2">
        <UCard class="chart-surface rounded-[1.75rem] shadow-card">
          <template #header>
            <div>
              <h2 class="font-display text-2xl text-default">Buddy boats</h2>
              <p class="mt-1 text-sm text-muted">
                Find and pin the boats this captain follows on the public profile.
              </p>
            </div>
          </template>
          ...
          <div class="space-y-4">
            <p class="text-sm text-muted">
              Search AIS Hub by MMSI or boat name, then publish the saved list as buddy boats on the
              captain page.
            </p>
        ...
        <UCard class="chart-surface rounded-[1.75rem] shadow-card">
          <template #header>
            <div>
              ...
              <p class="mt-1 text-sm text-muted">
                Public vessel posture and discoverability controls.
              </p>
            </div>
          </template>
