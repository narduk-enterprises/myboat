...
const publicVessels = computed(
  () => overview.value?.vessels.filter((vessel) => vessel.sharePublic) || [],
)
const privateVessels = computed(
  () => overview.value?.vessels.filter((vessel) => !vessel.sharePublic) || [],
)
        ...
        <UCard class="border-default/80 bg-default/90 shadow-card">
          ...
          <div class="space-y-4 text-sm text-muted">
            <div class="rounded-2xl border border-default bg-elevated/60 px-4 py-4">
              ...
              <p class="mt-1">
                Nothing public except what you explicitly publish through a profile.
              </p>
            </div>
            <div class="rounded-2xl border border-default bg-elevated/60 px-4 py-4">
            ...
            <div class="rounded-2xl border border-default bg-elevated/60 px-4 py-4">
              ...
              <p class="mt-1">
                Exact live sharing is allowed per vessel, with a captain-selectable bundle.
              </p>
            </div>
          </div>
        </UCard>
            ...
            <div class="space-y-3">
              <div class="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p class="mt-1 text-sm text-muted">
                    Private vessel, not discoverable in explore.
                  </p>
                </div>
